...
# 1. GENERATE raw_extracted_data.json
print("Building raw_extracted_data.json...")
raw_extracted = {
    "sourceExport": {
        "exportPath": "ChatExport_2026-08-14",
        "totalMessages": len(raw_messages),
        "totalFiles": len(all_disk_files),
        "totalPdfs": len(pdf_disk_files)
    },
...
# 2. GENERATE file_inventory.json
print("Building file_inventory.json...")
# Map of day to PDF filename(s)
day_to_pdf_map = {
    ...
    6: ["A2SV G5 Remote Focus, time management and planning.pdf"], # referenced in export, file exceeded export size
...
# Reverse lookup for day from pdf
pdf_to_day_map = {}
...
for fn in all_disk_files:
    ...
    file_inventory.append({
        ...
        "associatedWeek": matched_day, # 1-to-1 week mapping
        "topic": topic,
        "confidence": "high" if matched_day else "medium",
        "notes": "Original Telegram export file"
    })

# Add referenced files not on disk (exceeded export size limits in Telegram)
missing_referenced = [
    {
...
# 3. GENERATE phases.json
print("Building phases.json...")
phases_data = [
    {
        "id": "foundation",
        "name": "Foundation",
...
print("Building curriculum and problems datasets...")
